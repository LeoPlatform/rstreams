...
Above is the ideal.  Yes, you can have hardcoded values in the config file since they are shared across so many places
in your code that it's convenient to just stick them in your config and they won't ever change across deployments in
different environments.  `numRetryAttempts` is just such and innocuous piece of
config that you can hardcode. The engineer who wrote the RStreams Flow Config library was concerned that if we left the ability to
...
and know how to turn `customer-db-password-prod` into the actual password.  That something is RStreams Flow Config.
